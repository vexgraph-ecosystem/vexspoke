...
2. **Status legend (canonical, mirrors `_checklist/README.md`):** 💚 implemented & polishing · 🟩 implemented · 🟨 incomplete but proven by tests · 🟧 draft (code/concept exists, not wired) · 🟥 in docs only (specced in `_docs/`/`_thoughts/`, zero source) · ⬜ not implemented (important future work, not started — ⬜ is never "dropped/archived"; it marks an important concept not yet built). A row's scope line is rewritten when the actor or deliverable changes, not just the emoji.
