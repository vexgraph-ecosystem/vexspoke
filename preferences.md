...
7. **The One Type Registry Rule (Uniform Per-Project Numbering)**:
   - Every repository owns exactly one class registry in its own `*-type.h` (vexspoke `oop/type.h`, graphvex `graphvex/type.h`, darling `c23/darling-type.h`). Repo-local class numbers start at **1** and may contain gaps — there are **no global hex windows**. The 64-bit id layout `0x F PRPR M W1 W2 PDPD CCCCCCCC` carries an 8-bit PROJECT byte (`PROJ_VEXSPOKE`, `PROJ_DARLING`, `PROJ_GRAPHVEX`, `PROJ_HOTCWAP`, ...) that is the only cross-project disambiguator: id #1 in darling ≠ id #1 in vexspoke.
