...
public final class IntFloat {
    ...
    public static float getFracPart(long pointer) {
        if (pointer == 0L) throw new NullPointerException("Accessing NULL off-heap pointer!");
        return ForeignMemory.unsafeGetFloat(pointer + 4L);
    }

    public static float getAsFloat(long pointer) {
        if (pointer == 0L) throw new NullPointerException("Accessing NULL off-heap pointer!");
        return ForeignMemory.unsafeGetInt(pointer) + ForeignMemory.unsafeGetFloat(pointer + 4L);
    }

    public static int getIntPart(long pointer, int index) {
        checkBounds(pointer, index);
        return ForeignMemory.unsafeGetInt(pointer + (index * 8L));
    }

    public static float getFracPart(long pointer, int index) {
        checkBounds(pointer, index);
        return ForeignMemory.unsafeGetFloat(pointer + (index * 8L) + 4L);
    }

    public static float getAsFloat(long pointer, int index) {
        checkBounds(pointer, index);
        return ForeignMemory.unsafeGetInt(pointer + (index * 8L)) + ForeignMemory.unsafeGetFloat(pointer + (index * 8L) + 4L);
    }
    ...
    public static void set(long pointer, int intPart, float fracPart) {
        ...
        ForeignMemory.unsafeSet(pointer + 4L, fracPart);
    }

    public static void set(long pointer, int index, int intPart, float fracPart) {
        checkBounds(pointer, index);
        if (classId(pointer) != CLASS_ID) throw new IllegalArgumentException("Pointer 0x" + java.lang.Long.toHexString(pointer).toUpperCase() + " is Class ID " + classId(pointer) + ", expected IntFloat (Class ID " + CLASS_ID + ")");
        ForeignMemory.unsafeSet(pointer + (index * 8L), intPart);
        ForeignMemory.unsafeSet(pointer + (index * 8L) + 4L, fracPart);
    }
    ...
    public static void unsafeSet(long pointer, int val1, float val2) {
        ...
        ForeignMemory.unsafeSet(pointer + 4L, val2);
    }

    @Unsafe
    public static void unsafeSet(long pointer, int index, int val1, float val2) {
        ForeignMemory.unsafeSet(pointer + (index * 8L), val1);
        ForeignMemory.unsafeSet(pointer + (index * 8L) + 4L, val2);
    }

    @Volatile
    public static int getIntPartVolatile(long pointer, int index) {
        checkBounds(pointer, index);
        return ForeignMemory.unsafeGetIntVolatile(pointer + (index * 8L));
    }

    @Volatile
    public static float getFloatPartVolatile(long pointer, int index) {
        checkBounds(pointer, index);
        return ForeignMemory.unsafeGetFloatVolatile(pointer + (index * 8L) + 4L);
    }

    @Volatile
    public static void setVolatile(long pointer, int index, int val1, float val2) {
        checkBounds(pointer, index);
        ForeignMemory.unsafeVolatileSet(pointer + (index * 8L), val1);
        ForeignMemory.unsafeVolatileSet(pointer + (index * 8L) + 4L, val2);
    }
