...
public final class Bool {
    ...
    private static final long POOLED_ARRAY_SIZE = 8L + (((DEFAULT_CAPACITY + 63) / 64) * 8L); // Pack 64 bools per long
    ...
    static {
        try {
            ...
            ARRAY_FREE_HEAD_VH = lookup.findStaticVarHandle(Bool.class, "arrayFreeHead", long.class);
            MATRIX_FREE_HEAD_VH = lookup.findStaticVarHandle(Bool.class, "matrixFreeHead", long.class);

            SINGLETON_EXPANDING_VH = lookup.findStaticVarHandle(Bool.class, "singletonExpanding", int.class);
            ARRAY_EXPANDING_VH = lookup.findStaticVarHandle(Bool.class, "arrayExpanding", int.class);
            MATRIX_EXPANDING_VH = lookup.findStaticVarHandle(Bool.class, "matrixExpanding", int.class);
    ...
    public static long expandArray(long oldPointer, int newLength) {
        ...
        int longsToCopy = (elementsToCopy + 63) / 64;
        ForeignMemory.copy(oldPointer, newPointer, longsToCopy * 8L);
        free(oldPointer);
        return newPointer;
    }
    ...
    public static boolean get(long pointer) {
        ...
        return ForeignMemory.getLong(pointer) != 0L;
    }

    public static boolean get(long pointer, int index) { 
        checkBounds(pointer, index);
        long wordOffset = index >> 6;
        long word = ForeignMemory.getLong(pointer + (wordOffset * 8L));
    ...
    public static void set(long pointer, boolean value) {
        ...
        ForeignMemory.putLong(pointer, value ? 1L : 0L);
    }

    public static void setTrue(long pointer, int index) { 
        checkBounds(pointer, index);
        long wordOffset = index >> 6;
        long mask = 1L << (index & 63);
        ForeignMemory.getAndBitwiseOrLong(pointer + (wordOffset * 8L), mask);
    }

    public static void setFalse(long pointer, int index) { 
        checkBounds(pointer, index);
        long wordOffset = index >> 6;
        long mask = ~(1L << (index & 63));
        ForeignMemory.getAndBitwiseAndLong(pointer + (wordOffset * 8L), mask);
    }
    ...
    private static void checkBounds(long pointer, int index) {
        ...
        if (index < 0 || index >= len) {
            throw new IndexOutOfBoundsException("Index " + index + " out of bounds for off-heap bool length " + len + " (Ptr: 0x" + java.lang.Long.toHexString(pointer).toUpperCase() + ", Type: 0x" + Integer.toHexString(type(pointer)).toUpperCase() + ")");
        }
    }
