...
public final class Short {
    ...
    static {
        try {
            ...
            MATRIX_FREE_HEAD_VH = lookup.findStaticVarHandle(Short.class, "matrixFreeHead", long.class);
            ...
            MATRIX_EXPANDING_VH = lookup.findStaticVarHandle(Short.class, "matrixExpanding", int.class);
    ...
    public static void set(long pointer, short value) {
        ...
        ForeignMemory.putShort(pointer, value);
    }
    ...
    private static void checkBounds(long pointer, int index) {
        ...
        if (index < 0 || index >= len) {
            throw new IndexOutOfBoundsException("Index " + index + " out of bounds for off-heap short length " + len + " (Ptr: 0x" + java.lang.Long.toHexString(pointer).toUpperCase() + ", Type: 0x" + Integer.toHexString(type(pointer)).toUpperCase() + ")");
        }
    }
    ...
    public static short getVolatile(long pointer, int index) {
        checkBounds(pointer, index);
        return ForeignMemory.getShortVolatile(pointer + (index * 2L));
    }
    ...
    public static void setVolatile(long pointer, int index, short value) {
        checkBounds(pointer, index);
        ForeignMemory.putShortVolatile(pointer + (index * 2L), value);
    }
