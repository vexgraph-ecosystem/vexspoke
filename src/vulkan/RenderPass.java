...
public final class RenderPass {
    ...
    @Required
    public static final int CLASS_ID = TypeRegister.ID_RENDER_PASS;

    public static final long MAX_VALUE = 9223372036854775807L;
    public static final long MIN_VALUE = -9223372036854775808L;

    public static final int TYPE_SINGLETON = TypeRegister.RENDER_PASS_SINGLETON; // 0xAA000002
    public static final int TYPE_ARRAY     = TypeRegister.RENDER_PASS_ARRAY;     // 0xBB000002
    public static final int TYPE_MATRIX    = TypeRegister.RENDER_PASS_POINTER;   // 0xCC000002
    ...
    static {
        try {
            ...
            SINGLETON_FREE_HEAD_VH = lookup.findStaticVarHandle(RenderPass.class, "singletonFreeHead", long.class);
            ARRAY_FREE_HEAD_VH = lookup.findStaticVarHandle(RenderPass.class, "arrayFreeHead", long.class);
            MATRIX_FREE_HEAD_VH = lookup.findStaticVarHandle(RenderPass.class, "matrixFreeHead", long.class);

            SINGLETON_EXPANDING_VH = lookup.findStaticVarHandle(RenderPass.class, "singletonExpanding", int.class);
            ARRAY_EXPANDING_VH = lookup.findStaticVarHandle(RenderPass.class, "arrayExpanding", int.class);
            MATRIX_EXPANDING_VH = lookup.findStaticVarHandle(RenderPass.class, "matrixExpanding", int.class);
    ...
    private static void checkActive() {
        if (!active) throw new IllegalStateException("RenderPass subsystem is not active!");
    }
    ...
    // --- VULKAN FACTORY ---
    public static long create(VkDevice device, VkRenderPassCreateInfo createInfo) {
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            set(enginePtr, pRenderPass.get(0));
        }
        return enginePtr;
    }
    ...
    public static void destroy(long enginePtr, VkDevice device) {
        ...
        if(vkHandle != 0L) {
            vkDestroyRenderPass(device, vkHandle, null);
            set(enginePtr, 0L);
        }
        free(enginePtr);
    }
