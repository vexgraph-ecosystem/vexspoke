...
public final class CommandPool {
    ...
    @Required
    public static final int CLASS_ID = TypeRegister.ID_COMMAND_POOL;

    public static final long MAX_VALUE = 9223372036854775807L;
    public static final long MIN_VALUE = -9223372036854775808L;

    public static final int TYPE_SINGLETON = TypeRegister.COMMAND_POOL_SINGLETON; // 0xAA000002
    public static final int TYPE_ARRAY     = TypeRegister.COMMAND_POOL_ARRAY;     // 0xBB000002
    public static final int TYPE_MATRIX    = TypeRegister.COMMAND_POOL_POINTER;   // 0xCC000002
    ...
    static {
        try {
            ...
            SINGLETON_FREE_HEAD_VH = lookup.findStaticVarHandle(CommandPool.class, "singletonFreeHead", long.class);
            ARRAY_FREE_HEAD_VH = lookup.findStaticVarHandle(CommandPool.class, "arrayFreeHead", long.class);
            MATRIX_FREE_HEAD_VH = lookup.findStaticVarHandle(CommandPool.class, "matrixFreeHead", long.class);

            SINGLETON_EXPANDING_VH = lookup.findStaticVarHandle(CommandPool.class, "singletonExpanding", int.class);
            ARRAY_EXPANDING_VH = lookup.findStaticVarHandle(CommandPool.class, "arrayExpanding", int.class);
            MATRIX_EXPANDING_VH = lookup.findStaticVarHandle(CommandPool.class, "matrixExpanding", int.class);
    ...
    private static void checkActive() {
        if (!active) throw new IllegalStateException("CommandPool subsystem is not active!");
    }
    ...
    // --- VULKAN FACTORY ---
    public static long create(VkDevice device, int queueFamilyIndex) {
        long enginePtr = allocateSingleton();
        try (MemoryStack stack = MemoryStack.stackPush()) {
            VkCommandPoolCreateInfo poolInfo = VkCommandPoolCreateInfo.calloc(stack)
                    .sType(VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO)
                    .queueFamilyIndex(queueFamilyIndex)
                    .flags(VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT); // Allow resetting buffers individually
    ...
    public static void destroy(long enginePtr, VkDevice device) {
        ...
        if(vkHandle != 0L) {
            vkDestroyCommandPool(device, vkHandle, null);
            set(enginePtr, 0L);
        }
        free(enginePtr);
    }
