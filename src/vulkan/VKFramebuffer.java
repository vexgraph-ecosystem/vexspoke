...
public final class VKFramebuffer {
    ...
    @Required
    public static final int CLASS_ID = TypeRegister.ID_VK_FRAMEBUFFER;

    public static final long MAX_VALUE = 9223372036854775807L;
    public static final long MIN_VALUE = -9223372036854775808L;

    public static final int TYPE_SINGLETON = TypeRegister.VK_FRAMEBUFFER_SINGLETON; // 0xAA000002
    public static final int TYPE_ARRAY     = TypeRegister.VK_FRAMEBUFFER_ARRAY;     // 0xBB000002
    public static final int TYPE_MATRIX    = TypeRegister.VK_FRAMEBUFFER_POINTER;   // 0xCC000002
    ...
    static {
        try {
            ...
            SINGLETON_FREE_HEAD_VH = lookup.findStaticVarHandle(VKFramebuffer.class, "singletonFreeHead", long.class);
            ARRAY_FREE_HEAD_VH = lookup.findStaticVarHandle(VKFramebuffer.class, "arrayFreeHead", long.class);
            MATRIX_FREE_HEAD_VH = lookup.findStaticVarHandle(VKFramebuffer.class, "matrixFreeHead", long.class);

            SINGLETON_EXPANDING_VH = lookup.findStaticVarHandle(VKFramebuffer.class, "singletonExpanding", int.class);
            ARRAY_EXPANDING_VH = lookup.findStaticVarHandle(VKFramebuffer.class, "arrayExpanding", int.class);
            MATRIX_EXPANDING_VH = lookup.findStaticVarHandle(VKFramebuffer.class, "matrixExpanding", int.class);
    ...
    private static void checkActive() {
        if (!active) throw new IllegalStateException("VKFramebuffer subsystem is not active!");
    }
    ...
    // --- VULKAN FACTORY ---
    public static long create(VkDevice device, long renderPassEnginePtr, LongBuffer attachments, int width, int height) {
        long enginePtr = allocateSingleton();
        try (MemoryStack stack = MemoryStack.stackPush()) {
            long vkRenderPass = RenderPass.get(renderPassEnginePtr);
            ...
            if (vkCreateFramebuffer(device, framebufferInfo, null, pFramebuffer) != VK_SUCCESS) {
                throw new RuntimeException("Failed to create VKFramebuffer!");
            }
            set(enginePtr, pFramebuffer.get(0));
        }
        return enginePtr;
    }
    ...
    public static void destroy(long enginePtr, VkDevice device) {
        ...
        if(vkHandle != 0L) {
            vkDestroyFramebuffer(device, vkHandle, null);
            set(enginePtr, 0L);
        }
        free(enginePtr);
    }
