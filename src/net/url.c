...
int64_t NetUrl_build(const char *scheme, const char *host, int port,
    ...
    int defaultPort = NetUrl_defaultPort(s);
...
int64_t NetUrl_basicAuth(const char *user, const char *pass, char *out, size_t cap) {
    ...
    char encoded[768];
    int64_t e = NetUrl_base64((const uint8_t *)creds, (size_t)c, encoded, sizeof(encoded));
    if (e < 0) return -1;

    return (int64_t)snprintf(out, cap, "Basic %s", encoded);
}

int64_t NetUrl_bearerAuth(const char *token, char *out, size_t cap) {
    return (int64_t)snprintf(out, cap, "Bearer %s", token ? token : "");
}
